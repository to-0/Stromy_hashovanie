...
// Zdroj https://github.com/amitbansal7/Data-Structures-and-Algorithms/blob/master/8.AVL-tree/AVLTree.c
#include "stdio.h"
#include "stdlib.h"
...
struct Node* Insert(struct Node* root,struct Node *n)
{
    ...
    // Left Left Case
    if(balance > 1 && n->key < root->left->key)
        return RightRotate(root);

    // Right Right Case
    if(balance < -1 && n->key > root->right->key)
        return LeftRotate(root);
